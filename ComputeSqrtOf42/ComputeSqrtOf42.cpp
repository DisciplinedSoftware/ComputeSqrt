...
public:
    ...
    static constexpr const extended_type base = extended_type{ 1 }
    << nb_extended_type_bits;
    ...
    constexpr large_integer(const char* str_)
        : large_integer(from_string(str_)) {}
    constexpr large_integer(const std::string& str_)
        : large_integer(from_string(str_)) {}
    ...
    [[nodiscard]] constexpr large_integer
        operator+(const large_integer& other_) const {
    ...
    [[nodiscard]] constexpr std::strong_ordering
        operator<=>(std::integral auto rhs_) const {
...
private:
    ...
    public:
        ...
        constexpr large_integer_data_ref(bool sign_,
            const std::vector<underlying_type>& data_)
            : sign(sign_), data(std::cref(data_)) {}

        [[nodiscard]] constexpr large_integer
            operator+(const large_integer_data_ref& other_) const {
            ...
            auto result_data =
                add_large_unsigned_integer_sorted(data.get(), other_.data.get());
        ...
        [[nodiscard]] constexpr large_integer
            ...
            bool result_sign = sign;
            auto result_data =
                subtract_large_unsigned_integer_sorted(data.get(), other_.data.get());
        ...
        [[nodiscard]] constexpr large_integer
            ...
            if (sign != other_.sign) {
                ...
                auto result =
                    *this * large_integer_data_ref(!other_.sign, other_.data.get());
            ...
            auto result_data =
                multiply_large_unsigned_integer_sorted(data.get(), other_.data.get());
        ...
        [[nodiscard]] constexpr std::strong_ordering
            operator<=>(const large_integer_data_ref& other_) const {
            if (sign != other_.sign) {
                return sign ? std::strong_ordering::greater
    ...
    [[nodiscard]] static constexpr collection_type
        to_data_collection(std::integral auto value_) {
    ...
    [[nodiscard]] static constexpr collection_type
        add_large_unsigned_integer_sorted(const collection_type& lhs_,
            const collection_type& rhs_) {
        assert(compare_large_unsigned_integer(lhs_, rhs_) ==
            std::strong_ordering::equal ||
            compare_large_unsigned_integer(lhs_, rhs_) ==
    ...
    [[nodiscard]] static constexpr collection_type
        subtract_large_unsigned_integer_sorted(const collection_type& lhs_,
            const collection_type& rhs_) {
        assert(compare_large_unsigned_integer(lhs_, rhs_) ==
            std::strong_ordering::equal ||
            compare_large_unsigned_integer(lhs_, rhs_) ==
    ...
    [[nodiscard]] static constexpr collection_type
        multiply_large_unsigned_integer_sorted(const collection_type& lhs_,
            const collection_type& rhs_) {
        assert(compare_large_unsigned_integer(lhs_, rhs_) ==
            std::strong_ordering::equal ||
            compare_large_unsigned_integer(lhs_, rhs_) ==
    ...
    [[nodiscard]] static constexpr std::strong_ordering
        compare_large_unsigned_integer(const collection_type& lhs_,
            const collection_type& rhs_) {
        if (lhs_.size() != rhs_.size()) {
            return (lhs_.size() < rhs_.size()) ? std::strong_ordering::less
                : std::strong_ordering::greater;
        }
    ...
    [[nodiscard]] static constexpr large_integer
        trim_upper_zeros(large_integer&& result_) {
...
std::strong_ordering operator<=>(const large_integer& lhs_,
    std::integral auto rhs_) {
    return lhs_ <=> large_integer(rhs_);
}

std::strong_ordering operator<=>(std::integral auto lhs_,
    const large_integer& rhs_) {
    return large_integer(lhs_) <=> rhs_;
}
...
namespace details {
    ...
    [[nodiscard]] constexpr std::string
        divide_integer_as_string_by_integer(const std::string& number,
            large_integer::extended_type divisor) {
    ...
    [[nodiscard]] constexpr large_integer::extended_type
        modulo_integer_as_string_by_integer(const std::string& num,
            large_integer::extended_type a) {
        // Initialize result
...
namespace details {
    ...
    [[nodiscard]] constexpr std::string
        add_integers_as_string(const std::string& lhs_, const std::string& rhs_) {
    ...
    [[nodiscard]] constexpr std::string
        multiply_integer_as_string_by_integer(const std::string& num_,
            large_integer::extended_type factor_) {
        std::string result;
    ...
    // Function to concatenate the vector elements into a single large integer
    // represented as a string
    [[nodiscard]] constexpr std::string concatenate_integers_to_string(
        const std::vector<large_integer::underlying_type>& data_) {
        std::string result = "0";
...
TEST_CASE("large_integer") {
    ...
    CHECK(to_string(large_integer(123456789012L) +
        large_integer(123456789012L)) == "246913578024"s);
    CHECK(to_string(large_integer(-123456789012L) +
        large_integer(-123456789012L)) == "-246913578024"s);
    CHECK(to_string(large_integer(123456789012L) +
        large_integer(-123456789012L)) == "0"s);
    CHECK(to_string(large_integer(-123456789012L) +
        large_integer(123456789012L)) == "0"s);
    CHECK(to_string(large_integer(123456789012L) +
        large_integer(-123456789000L)) == "12"s);
    CHECK(to_string(large_integer(246913578024L) +
        large_integer(-123456789012L)) == "123456789012"s);
    CHECK(to_string(large_integer(-246913578024L) +
        large_integer(123456789012L)) == "-123456789012"s);
    CHECK(to_string(large_integer(-123456789012L) +
        large_integer(246913578024L)) == "123456789012"s);
    CHECK(to_string(large_integer(123456789012L) +
        large_integer(-246913578024L)) == "-123456789012"s);

    CHECK(to_string(
        large_integer("42010168383160134110440665745547766649977556245")) ==
        "42010168383160134110440665745547766649977556245"s);
    CHECK(to_string(large_integer(
        "-42010168383160134110440665745547766649977556245")) ==
        "-42010168383160134110440665745547766649977556245"s);

    CHECK(to_string(
        large_integer("42010168383160134110440665745547766649977556245") -
        large_integer("42010168383160134110440665745547766649977556200")) ==
        "45"s);

    CHECK(to_string(large_integer(246913578024L) *
        large_integer(123456789012L)) == "30483157506306967872288"s);
    CHECK(to_string(large_integer(-246913578024L) *
        large_integer(-123456789012L)) == "30483157506306967872288"s);
    CHECK(
        to_string(large_integer(246913578024L) * large_integer(-123456789012L)) ==
        "-30483157506306967872288"s);
    CHECK(to_string(large_integer(-246913578024L) *
        large_integer(123456789012L)) == "-30483157506306967872288"s);
    CHECK(to_string(large_integer(-123456789012L) *
        large_integer(246913578024L)) == "-30483157506306967872288"s);
    CHECK(
        to_string(large_integer(123456789012L) * large_integer(-246913578024L)) ==
        "-30483157506306967872288"s);

    CHECK(
        large_integer("42010168383160134110440665745547766649977556245") *
        large_integer("1234567890987654321") ==
        large_integer(
            "51864404980834242630409449768792397904982098404496001028394784645"));

    CHECK(large_integer(
        "677957529792349324789802941828181753767622738062474781504901373241"
        "153594786016563107585657956824623391081159160787412056366438864227"
        "937145739025985756896095893577290800904801110410474661743617925268"
        "4469483776429833549880669503680760948677500") -
        large_integer(
            "64807406984078602309659674360879966577052043070583465497113543"
            "97809617377844044371400360906605610235675450542097411694335491"
            "91340490660868894581896166467395130558522782263609566882268066"
            "8761521776633672599142812990432160139844957280499363525") ==
        large_integer(
            "298834599515633016932061982193820879971023073566401265337659334601"
            "918570016121259675495672962635998235361049510462426228172475237372"
            "762768444440895904287007630187680186411915435282065948674657476050"
            "796884633616843117720529658723480449313975"));
}
...
// ----------------------------------------------------------------------------
// This method has been simplified and only supports computing the square root
// of integer value
std::string
compute_square_root_digit_by_digit_method(std::integral auto value_,
    unsigned int max_precision_) {
    ...
    std::string result_string(std::begin(integral_string_rng),
        std::end(integral_string_rng));
    ...
    while (precision > 0) {
        result_string +=
            static_cast<std::string::value_type>(compute_next_digit(next_value)) +
            '0';
    ...
    const double rounded_last_digit =
        std::round(last_digit + (static_cast<double>(rounding_digit) / 10.0));

    if (rounded_last_digit >= 10) {
        result_string.back() = '0';
        bool carry = true;
        for (auto& digit :
            result_string | std::views::reverse | std::views::drop(1) |
        ...
        if (carry) {
            ...
            const auto it =
                std::find(std::begin(result_string), std::end(result_string), '.');
        ...
        result_string.back() =
            static_cast<std::string::value_type>(rounded_last_digit) + '0';
    }
...
TEST_CASE("compute_square_root_digit_by_digit_method") {
    ...
    CHECK(compute_square_root_digit_by_digit_method(2, 32) ==
        "1.4142135623730950488016887242097"s);

    CHECK(compute_square_root_digit_by_digit_method(42, 31) ==
        "6.480740698407860230965967436088"s);
    CHECK(
        compute_square_root_digit_by_digit_method(42, 100) ==
        "6.480740698407860230965967436087996657705204307058346549711354397809617377844044371400360906605610236"s);
...
int main(int argc, const char* argv[]) {
    ...
    // Adjust the number of digits to the number of significant digits for a long
    // double
    ...
    std::cout
        << ++counter
        << ". Using stream operator with a constexpr constant as an integer: "
        << sqrt42<int> << '\n';
    std::cout << ++counter
        << ". Using stream operator with a constexpr constant as a float: "
        << sqrt42<float> << '\n';
    std::cout << ++counter
        << ". Using stream operator with a constexpr constant as a double: "
        << sqrt42<double> << '\n';
    std::cout
        << ++counter
        << ". Using stream operator with a constexpr constant as a long double: "
        << sqrt42<long double> << '\n';

    print_using_std_printf(
        "%d. Using std::printf with fix-point notation and float: %.9f\n",
        ++counter,
        sqrt42<float>); // This is useless as f consider the argument as a double
    print_using_std_printf(
        "%d. Using std::printf with fix-point notation and double: %.17lf\n",
        ++counter, sqrt42<double>);
    print_using_std_printf(
        "%d. Using std::printf with fix-point notation and long double: %.17Lf\n",
        ++counter, sqrt42<long double>);

    print_using_std_printf(
        "%d. Using std::printf with scientific notation with float: %.9e\n",
        ++counter,
        sqrt42<float>); // This is useless as f consider the argument as a double
    print_using_std_printf(
        "%d. Using std::printf with scientific notation with double: %.17le\n",
        ++counter, sqrt42<double>);
    print_using_std_printf("%d. Using std::printf with scientific notation with "
        "long double: %.17Le\n",
        ++counter, sqrt42<long double>);

    print_using_std_printf(
        "%d. Using std::printf with hexadecimal notation with float: %a\n",
        ++counter, sqrt42<float>);
    print_using_std_printf(
        "%d. Using std::printf with hexadecimal notation with double: %la\n",
        ++counter, sqrt42<double>);
    print_using_std_printf(
        "%d. Using std::printf with hexadecimal notation with long double: %La\n",
        ++counter, sqrt42<long double>);

    std::cout << ++counter << ". Using stream operator with a std::sqrt(float): "
        << std::sqrt(42.0f) << '\n';
    std::cout << ++counter << ". Using stream operator with a std::sqrt(double): "
        << std::sqrt(42.0) << '\n';
    std::cout << ++counter
        << ". Using stream operator with a std::sqrt(long double): "
        << std::sqrt(42.0l) << '\n';
    std::cout << ++counter << ". Using stream operator with a std::sqrtf(float): "
        << std::sqrt(42.0f) << '\n';
    std::cout << ++counter
        << ". Using stream operator with a std::sqrtl(long double): "
        << std::sqrt(42.0l) << '\n';

    std::cout << std::format(
        "{:}. Using std::format with fix-point notation and float: {:.9f}\n",
        ++counter, sqrt42<float>);
    std::cout << std::format(
        "{:}. Using std::format with fix-point notation and double: {:.17f}\n",
        ++counter, sqrt42<double>);
    std::cout << std::format("{:}. Using std::format with fix-point notation and "
        "long double: {:.17f}\n",
        ++counter, sqrt42<long double>);

    std::cout << std::format(
        "{:}. Using std::format with scientific notation and float: {:.9e}\n",
        ++counter, sqrt42<float>);
    std::cout << std::format(
        "{:}. Using std::format with scientific notation and double: {:.17e}\n",
        ++counter, sqrt42<double>);
    std::cout << std::format("{:}. Using std::format with scientific notation "
        "and long double: {:.17e}\n",
        ++counter, sqrt42<long double>);

    std::cout << std::format(
        "{:}. Using std::format with hexadecimal notation and float: {:a}\n",
        ++counter, sqrt42<float>);
    std::cout << std::format(
        "{:}. Using std::format with hexadecimal notation and double: {:a}\n",
        ++counter, sqrt42<double>);
    std::cout << std::format("{:}. Using std::format with hexadecimal notation "
        "and long double: {:a}\n",
        ++counter, sqrt42<long double>);
    ...
    std::cout << ++counter
        << ". Using custom function using constexpr Heron's method: "
        << constexpr_type_v<compute_square_root_heron_method_constexpr(
            42)> << '\n';

    std::cout << ++counter << ". Using custom function using Newton's method: "
        << compute_square_root_binary_search_method(42) << '\n';
    std::cout << ++counter << ". Using custom function using Heron's method: "
        << compute_square_root_heron_method(42) << '\n';
    std::cout << ++counter << ". Using custom function using Bakhshali's method: "
        << compute_square_root_bakhshali_method(42) << '\n';
    std::cout << ++counter << ". Using infinite digits (only show 1k): "
        << compute_square_root_digit_by_digit_method(42, 1'000) << '\n';
